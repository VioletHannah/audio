#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2025/7/11 16:23
# @Author : 箴澄
# @Func：改为更大的3D卷积网络
# @File : MultiSource_3DCNN_mapNet_revise.py
# @Software: PyCharm
...
class MultiSource3DCNNMapNetRevise(nn.Module):
    def __init__(self):
        super(MultiSource3DCNNMapNetRevise, self).__init__()
        # [B, 1, 16000, 8, 8]
        # 3D卷积层
        self.Conv3Dstack1 = nn.Sequential(
            nn.Conv3d(1, 2, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=(3, 1, 1)),
            nn.BatchNorm3d(2),
            nn.LeakyReLU(0.01, inplace=True),
            nn.Conv3d(2, 4, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=(3, 1, 1)),
            nn.BatchNorm3d(4),
            nn.LeakyReLU(0.01, inplace=True),
        ...
        self.Conv3Dstack2 = nn.Sequential(
            nn.Conv3d(8, 16, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=(3, 1, 1)),
            nn.BatchNorm3d(16),
            nn.LeakyReLU(0.01, inplace=True),
            nn.Conv3d(16, 32, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=(3, 1, 1)),
            nn.BatchNorm3d(32),
            nn.LeakyReLU(0.01, inplace=True),
        ...
        self.Conv3Dstack3 = nn.Sequential(
            nn.Conv3d(64, 128, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=(3, 1, 1)),
            nn.BatchNorm3d(128),
            nn.LeakyReLU(0.01, inplace=True),
...
# m = MultiSource3DCNNMapNetRevise()
# x = torch.randn(2, 1, 16000, 8, 8)
# y = m(x)
# print(y.shape)  # 应该输出 [2, 128, 128]