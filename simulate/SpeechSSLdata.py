...
def generate_sound_source_localization_dataset(
        ...
        snr=10,  # 信噪比
        signal="/home/zengkehan/voice/google_speech_commands",  # 声源文件夹
        output="/home/zengkehan/voice/speech_snr_10_L"  # 输出文件夹
):
    """
    生成单声源直达声定位数据集
    """
    ...
    while sample_idx < num_samples:
        ...
        for i in range(multichannel_signal.shape[0]):
            ...
            if snr > 0:
                # 添加高斯噪声
                multi_audio = add_gaussian_noise(multichannel_signal[i], snr=snr)
            # 归一化到
            multi_audio = multi_audio / np.max(np.abs(multi_audio))
            sf.write(wav_filename, multi_audio, fs)
...
if __name__ == '__main__':
    # np.random.seed(42)
    generate_sound_source_localization_dataset(mic_length=0.15,
                                               output="/home/zengkehan/voice/speech_snr_10_S",)
