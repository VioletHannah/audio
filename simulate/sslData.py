#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2025-03-04 16:00
# @Author : 箴澄
# @Func : 使用Pyroomacoustics库生成声源定位的仿真数据
# @File : sound-source-localization-dataset.py
# @Software: PyCharm
...
def generate_sound_source_localization_dataset(
        ...
        mic_array_height=0.0,  # 麦克风阵列置于地面
        mic_length=0.3,
        mic_num_per_line=4
):
    """
    ...
    # os.makedirs('./voice/sin64_dataset/wavs', exist_ok=True)
    # os.makedirs('./voice/sin64_dataset/metadata', exist_ok=True)
    ...
    while sample_idx < num_samples:
        ...
        # 获取该样本的文件路径
        # audio_file = random_sample.meta.file_loc

        # 读取音频文件
        random_file = random.choice(flac_files)
        random_file_path = os.path.join(signal_folder, random_file)
        taudio, fs = sf.read(random_file_path)
        # 取左声道
        try :
            if taudio.shape[1] == 2:
                audio = taudio[:, 0]
            elif taudio.shape[1] > 2:
                continue
        except:
            audio = taudio
        ...
        # 保存 WAV 文件
        # room_dir = f'sin64_dataset/wavs/sample_{sample_idx}'
        ...
        for i in range(multichannel_signal.shape[0]):
            ...
            sf.write(wav_filename, multichannel_signal[i], fs)
        ...
        # json_filename = f'sin64_dataset/metadata/sample_{sample_idx}.json'
